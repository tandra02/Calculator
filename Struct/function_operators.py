...
def struct(list_value):
    ...
    # Highest prority
    list_value = process_operators(list_value, ['^', 'pow'])
...
print(struct([3, '^', 4, 'div', 2, 'pow', 3]))   # Expected output: ['div', ['^', 3, 4], ['pow', 2, 3]]
print(struct([4, '*', 2, 'pow', 3]))             # Expected output: ['*', 4, ['pow', 2, 3]]
print(struct([9, '*', 6, '^', 2, 'add', 4]))     # Expected output: ['add', ['*', 9, ['^', 6, 2]], 4]